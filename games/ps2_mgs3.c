...
static void PS2_MGS3_Inject(void)
{
	...
	viewBase = PS2_MEM_ReadPointer(MGS3_VIEWBASE);
	viewState = PS2_MEM_ReadUInt8(MGS3_VIEWSTATE);

	float fov = PS2_MEM_ReadFloat(viewBase + MGS3_FOV);
	float looksensitivity = (float)sensitivity;

	//while not aiming
	float camY = PS2_MEM_ReadFloat(viewBase + MGS3_CAMY);
	uint16_t camX = PS2_MEM_ReadUInt(viewBase + MGS3_CAMX);

	//while aiming
	uint16_t aimY = PS2_MEM_ReadUInt16(viewBase + MGS3_AIMY);
	uint16_t aimX = PS2_MEM_ReadUInt16(viewBase + MGS3_AIMX);
	...
	float dy = ym * looksensitivity / fov;
	AccumulateAddRemainder(&aimYf, &yAccumulator, ym, dy);

	float dx = (float)-xmouse * looksensitivity / fov;
	...
	if (viewState == 0x00){
		PS2_MEM_WriteFloat(viewBase + MGS3_CAMY, camY);
		PS2_MEM_WriteUInt16(viewBase + MGS3_CAMX, camX);
	}
	
	if (viewState == 0x04 ||// first person
		viewState == 0x14){ //binoculars/scope
		PS2_MEM_WriteUInt16(viewBase + MGS3_AIMY, (uint16_t)aimYf);
		PS2_MEM_WriteUInt16(viewBase + MGS3_AIMX, (uint16_t)aimXf);
	}
}